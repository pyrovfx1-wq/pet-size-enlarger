-- Combined Script: Delta Warning + Pet Enlarger
-- TOCHIPYRO GUI
...
TitleLabel.Text = "TOCHIPYRO - Delta Warning"
...
-- Content Frame (where Pet Enlarger stuff lives)
...
local function enlargePet(petName)
	for _, pet in ipairs(workspace:GetChildren()) do
		if pet:IsA("Model") and pet.Name:lower() == petName:lower() then
			local originalSize = pet:GetExtentsSize()
			local goalSize = originalSize * 3 -- scale up by 3x
